...
| Share extension | 🟢 VERIFIED in sim | **Part B.2 working end-to-end on the iOS simulator:** Safari → Share → "Silo" → SwiftUI confirm sheet (URL + category) → Add → item lands in Stacks (Video, real thumbnail). Native sheet hands off via the **App Group queue** (openURL is blocked from extensions) → app **drains on foreground** → same extractor pipeline. Requires `expo prebuild --clean` (run:ios merge doesn't embed the target) + `ios.appleTeamId` for device/EAS. |
