...
interface ChatMessage {
  ...
  retryQuery?: string;
  /** The failure was the premium gate — offer the paywall, not a retry. */
  isPaywall?: boolean;
}
...
function makeDynamicStyles(c: ThemeColors) {
  return {
    ...
    paywallBubble: { backgroundColor: c.brandSoft, borderColor: c.brandBorder },
    paywallLabel: { color: c.textBrand },
    unlockButton: { backgroundColor: c.brand },
...
export default function ChatBot({ visible, onClose, prefill }: ChatBotProps) {
  ...
  const runQuery = useCallback(async (question: string) => {
    ...
    try {
      ...
      const paywalled = isPremiumRequired(error);
      setMessages((prev) => [
        ...prev,
        {
          id: nextId(),
          text: paywalled
            ? 'The assistant is part of Silo Premium. Everything you save, and the whole resurfacing loop, stays free.'
          ...
          isError: !paywalled,
          isPaywall: paywalled,
          retryQuery: paywalled ? undefined : question,
        },
      ]);
    } finally {
      setThinking(null);
                ...
                {messages.map((message) => (
                  ...
                  >
                      ...
                      style={[
                        ...
                        message.isUser
                          ...
                          : message.isError
                            ...
                            : message.isPaywall
                              ? [styles.errorBubble, dyn.paywallBubble]
                    ...
                    >
                      ...
                      {message.isPaywall && (
                        <View style={styles.errorHeader}>
                          <Ionicons name="sparkles" size={15} color={c.textBrand} />
                          <Text style={[styles.errorLabel, dyn.paywallLabel]}>Premium</Text>
                        </View>
                      )}
                      ...
                      {message.isPaywall && (
                        <PressableScale
                          ...
                          style={[styles.unlockButton, dyn.unlockButton]}
                          onPress={() => router.push('/paywall')}
                        >
                          <Text style={styles.unlockText}>See Premium</Text>
                        </PressableScale>
                      )}
                    </View>
